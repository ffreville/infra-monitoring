          ...
          <tr v-for="cluster in availableClusters" :key="cluster.id">
            ...
            <td class="px-3 py-2 whitespace-nowrap">
              <span 
                ...
                :class="getVersionClass(statefulset.clusterVersions[cluster.id])"
              >
                {{ formatVersion(statefulset.clusterVersions[cluster.id]?.version) }}
              </span>
            </td>
            <td class="px-3 py-2 whitespace-nowrap">
              <span 
                class="text-xs font-medium"
                :class="getReadyClass(statefulset.clusterVersions[cluster.id])"
              >
                {{ formatReady(statefulset.clusterVersions[cluster.id]) }}
              </span>
            </td>
          </tr>
        </tbody>
...
const hasUpdate = computed(() => {
  if (!props.checkForUpdates || !props.statefulset.version) {
    return false
  }
  
  const versionKey = `${props.statefulset.kind.toLowerCase()}-${props.statefulset.name}-${props.statefulset.namespace}`
  return props.versionUpdates[versionKey] !== undefined
})
...
const latestVersion = computed(() => {
  ...
  const versionKey = `${props.statefulset.kind.toLowerCase()}-${props.statefulset.name}-${props.statefulset.namespace}`
  return props.versionUpdates[versionKey] || ''
})
...
function getStatusClass(status) {
  ...
  return {
    ...
    'text-yellow-600': statusLower === 'pending' || statusLower === 'updating',
